...
export function calculatePriorityScore(complaint: PriorityInput): PriorityResult {
  ...
  const upvoteScore = Math.min(complaint.upvoteCount * 2, 30);
  const severityBonus = getSeverityBonus(text);
  const days = Math.floor((Date.now() - complaint.createdAt.getTime()) / 86400000);
  const ageScore = days * 5;
  ...
  console.log(`Priority calculated: score=${score}, priority=${priority}`);
