...
export default function AuthorityDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: complaints.length, pending, inProgress, resolvedToday, breached }
  }, [complaints])

  const queueComplaints = useMemo(() => {
    let active = complaints.filter((c) => c.status !== "resolved")
    if (filterBreachedOnly) active = active.filter((c) => c.sla?.breached)
    return active.sort((a, b) => b.priorityScore - a.priorityScore)
  }, [complaints, filterBreachedOnly])

  const visibleQueue = useMemo(() => queueComplaints.slice(0, limitCount), [queueComplaints, limitCount])
  ...
  return (
    <main className="space-y-6 p-6 max-w-7xl mx-auto pt-24 min-h-screen">
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between border-b pb-4 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Authority Dashboard</h1>
        ...
        <div className="flex items-center gap-3">
          {currentTime && (
            <span className="text-xs font-bold text-gray-400 bg-gray-100 px-3 py-1.5 rounded-full uppercase tracking-widest">{currentTime}</span>
          )}
          <Button size="sm" onClick={() => router.push("/authority/complaints")} className="font-bold text-xs uppercase tracking-wider">
      ...
      <section className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card className="bg-white border border-gray-100 shadow-sm text-center py-4 cursor-pointer hover:border-blue-300" onClick={() => router.push("/authority/complaints")}>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 border rounded-none transition ${
                filterBreachedOnly ? "bg-red-600 text-white border-red-600" : "bg-white text-gray-600 border-gray-300 hover:bg-red-50"
              }`}
            >
              {filterBreachedOnly ? "Showing: Breached Only" : "Filter: Breached Only"}
            </button>
            <Button variant="outline" size="sm" onClick={() => router.push("/authority/complaints")} className="text-[10px] font-bold uppercase tracking-wider">
              Full Queue
            </Button>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="shrink-0">
                    {isBreached ? (
                      <span className="text-[10px] font-extrabold text-red-600 bg-red-100 border border-red-200 px-2 py-0.5 uppercase tracking-wide">BREACHED</span>
                    ) : (
