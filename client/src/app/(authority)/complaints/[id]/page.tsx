...
export default function AuthorityComplaintAction({ params }: PageProps) {
  ...
  useEffect(() => {
    if (complaint?.title) {
      document.title = `NagarWatch Authority - ${complaint.title}`
    }
  }, [complaint])
  ...
  const handleMarkInProgress = async () => {
    ...
    try {
      const response = await complaintsAPI.updateStatus(id, {
        status: "in_progress",
        note: statusNote || "Issue has been accepted and is currently in progress of resolution.",
      })
  ...
  // 5. Resolve complaint action
  const handleResolve = async () => {
    if (resolving) return
    if (!afterImage) {
      setErrorMessage("After image required to prove resolution.")
      return
    }
    setResolving(true)
    setErrorMessage(null)
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8 pt-24 space-y-6 min-h-screen">
      ...
      <nav className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <Button
          onClick={() => router.push("/authority/complaints")}
          variant="ghost"
          className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-gray-500 hover:text-gray-900 px-0 self-start"
        >
      ...
      {/* Success/Error dismissible alerts */}
      {successMessage && (
      ...
      {errorMessage && (
        <div className="flex items-center gap-2 bg-red-600 text-white px-4 py-2.5 shadow border border-red-700 text-sm font-medium rounded-md animate-in fade-in slide-in-from-top-4">
          ...
          <button onClick={() => setErrorMessage(null)} className="text-white font-bold ml-2 text-xs uppercase">
            Dismiss
          </button>
        </div>
      )}
        ...
        <CardContent className="p-0 space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2 items-center mt-3">
              <Badge variant="outline" className="border-gray-300 text-gray-700 font-semibold uppercase">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-t pt-4 gap-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">
            <div>
              Submitted by: <strong className="text-gray-900 font-bold">{complaint.submittedBy?.name || "Anonymous"}</strong> &middot; {timeAgo(complaint.createdAt)}
              {complaint.ward && (
                <> &middot; Ward: <strong className="text-gray-900 font-bold">{complaint.ward.name}</strong></>
              )}
            </div>
            <div>
              Priority Score: <strong className="text-primary font-bold">{complaint.priorityScore}</strong> &middot; Upvotes: <strong className="text-gray-900 font-bold">{complaint.upvoteCount}</strong>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <section className="lg:col-span-2 space-y-4">
          ...
          {/* ACTION CARDS (Pending/In Progress) */}
          {complaint.status !== "resolved" && (
            <>
              {/* Mark as In Progress */}
                  ...
                  <CardContent className="p-6 space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        ...
                        placeholder="Add a note about action being taken..."
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-center gap-3">
                          <label
                            ...
                            className="text-xs text-primary font-bold hover:underline cursor-pointer uppercase tracking-wider"
                          >
                            Change Photo
                          </label>
                          <button
            ...
            <Card className="border border-green-200 bg-green-50/20 shadow-sm p-6 text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 mt-0.5">
                  This complaint was resolved on {formatDateTime(complaint.resolvedAt!)}
                </p>
              </div>

              {complaint.resolutionNote && (
                <div className="bg-white border border-green-100 p-4 rounded-lg text-left text-xs text-gray-700 italic">
