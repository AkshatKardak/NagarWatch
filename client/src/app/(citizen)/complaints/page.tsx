...
export default function CitizenComplaintsHistory() {
  ...
  // SEO
  useEffect(() => {
    document.title = "NagarWatch - My Complaints"
  }, [])
    ...
    <main className="space-y-6 p-6 max-w-7xl mx-auto pt-24 min-h-screen">
      ...
      <header className="flex items-center justify-between border-b pb-4 gap-4">
        ...
        <Button
          ...
          size="sm"
          className="bg-primary text-white font-bold flex items-center gap-1.5"
        >
        ...
        {(["all", "pending", "in_progress", "resolved"] as const).map((status) => {
          ...
          return (
            <Button
              ...
              className="text-xs uppercase tracking-wider font-semibold py-1.5 px-3.5 h-8 flex items-center gap-1.5"
            >
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1 max-w-xs mx-auto">
                {statusFilter === "all"
                  ...
                  : `You don't have any reports currently marked as ${statusFilter.replace("_", " ")}.`}
              </p>
            </div>
        ...
        <div className="space-y-4">
          {displayComplaints.map((c) => {
            ...
            return (
              <Card
                key={c._id}
                className="hover:shadow-md transition bg-white border border-gray-200/60 p-4"
              >
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex flex-wrap gap-1.5 items-center">
                      <h3
                        ...
                        className="font-bold text-sm text-gray-900 truncate hover:text-primary cursor-pointer flex-1 min-w-0"
                      >
                        {c.title}
                      </h3>
                      <div className="flex gap-1">
                        <Badge variant="outline" className="text-[10px] px-2 border-gray-300 text-gray-600 font-semibold uppercase">
                          {getCategoryLabel(c.category)}
                        </Badge>
                        <Badge
                          variant="secondary"
                          className={`text-[10px] px-2 font-semibold uppercase ${getPriorityColor(c.priority)} bg-white border`}
                        >
                          {c.priority}
                        </Badge>
                      </div>
                    ...
                    {c.status !== "resolved" && (
                      <div className="pt-1.5">
                        {slaLeft.isOverdue ? (
                          <span className="inline-flex items-center gap-1 text-[9px] font-extrabold uppercase px-2 py-0.5 bg-red-100 text-red-700 border border-red-200">
                            ⚠ Overdue
                          </span>
                        ) : slaLeft.percentage > 80 ? (
                          <span className="inline-flex items-center gap-1 text-[9px] font-extrabold uppercase px-2 py-0.5 bg-orange-100 text-orange-700 border border-orange-200">
                            ⏰ {slaLeft.hours}h left
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[9px] font-extrabold uppercase px-2 py-0.5 bg-green-100 text-green-700 border border-green-200">
                            ✓ {slaLeft.hours}h left
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex sm:flex-col items-start sm:items-end justify-between sm:justify-center gap-2 w-full sm:w-auto shrink-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                    <span
                      className={`px-2.5 py-0.5 text-xs font-bold shrink-0 uppercase tracking-wider ${getStatusColor(c.status)}`}
                    >
                      {c.status.replace("_", " ")}
                    </span>
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => router.push(`/complaints/${c._id}`)}
                      className="text-xs font-bold uppercase tracking-wider h-8 border-gray-300 hover:bg-gray-50 self-end"
                    >
                      View Details
                    </Button>
                  </div>
